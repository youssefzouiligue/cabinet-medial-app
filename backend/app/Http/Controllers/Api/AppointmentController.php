...
class AppointmentController extends Controller
{
    ...
    /**
     * Le médecin définit ou met à jour le lien de l'appel vidéo.
     * Accessible uniquement sur les rendez-vous de type "video" qui lui appartiennent.
     */
    public function setLienVideo(Request $request, Appointment $appointment)
    {
        if ($appointment->medecin_id !== $request->user()->id) {
            return response()->json(['message' => 'Accès refusé.'], 403);
        }

        if ($appointment->type_consultation !== 'video') {
            return response()->json(['message' => 'Ce rendez-vous n\'est pas de type vidéo.'], 422);
        }

        $validator = Validator::make($request->all(), [
            'lien_video' => 'required|url|max:500',
        ]);
        ...
        $appointment->update(['lien_video' => $request->lien_video]);

        return response()->json([
            'message'    => 'Lien vidéo mis à jour.',
            'lien_video' => $appointment->lien_video,
        ]);
    }
}
